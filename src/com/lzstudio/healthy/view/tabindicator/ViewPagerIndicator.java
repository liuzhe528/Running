package com.lzstudio.healthy.view.tabindicator;
...
public class ViewPagerIndicator extends LinearLayout {
	 ...
	 * tab上的内容
	 */
	private List<Colums> mTabTitles;
	/**
	 * 与之绑定的ViewPager
	 */
	 ...
	 * @param datas
	 */
	public void setTabItemTitles(List<Colums> datas) {
		// 如果传入的list有值，则移除布局文件中设置的view
		if (datas != null && datas.size() > 0) {
			this.removeAllViews();
			this.mTabTitles = datas;

			for (Colums colum : mTabTitles) {
				// 添加view
				addView(generateTextView(colum.getName()));
			}
			// 设置item的click事件
			setItemClickEvent();
		}
